                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="card card-body">
                        ...
                        <div class="space-y-4">
                            ...
                            @if($booking->booking_mode && $booking->booking_mode->value !== 'rental')
                                <div class="flex justify-between items-center py-3 border-t border-slate-100">
                                    <span class="text-slate-600">Modalita</span>
                                    <span class="font-semibold text-slate-900">{{ $booking->booking_mode->label() }}</span>
                                </div>
                                @if($booking->nodi_amount)
                                    <div class="flex justify-between items-center py-3 border-t border-slate-100">
                                        <span class="text-slate-600">Nodi</span>
                                        <x-nodi-badge :amount="$booking->nodi_amount" size="sm" />
                                    </div>
                                @endif
                            @endif
                        ...
                        @if($booking->status->value === 'completed' && !$booking->review)
                            <div class="mt-6 pt-4 border-t border-slate-100">
                                <a href="{{ route('my-bookings.review', $booking) }}" class="btn-primary inline-flex items-center gap-2">
                        ...
                        @if($booking->review)
                            <div class="mt-6 pt-4 border-t border-slate-100">
                                <div class="flex items-center gap-3">
                                    <span class="text-sm font-medium text-slate-600">La tua recensione:</span>
                                    <x-review-stars :rating="$booking->review->average_rating" size="sm" />
                                    @if($booking->review->is_verified)
                                ...
                                @if($booking->review->comment)
                                    <p class="text-sm text-slate-500 mt-2">{{ $booking->review->comment }}</p>
                                @endif
                            </div>
                        @endif
                    </div>

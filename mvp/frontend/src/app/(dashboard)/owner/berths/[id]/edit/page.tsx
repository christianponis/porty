...
export default function EditBerthPage() {
  ...
  useEffect(() => {
    Promise.all([ownerApi.getBerth(berthId), catalogApi.getPorts()])
      .then(([berth, portsData]) => {
        setPorts(portsData);
        setForm({
          name: berth.name,
          description: berth.description || '',
          port_id: String(berth.port.id),
          max_length: String(berth.max_length),
          max_beam: String(berth.max_beam),
          max_draft: String(berth.max_draft),
          price_per_night: String(berth.price_per_night),
          price_per_month: '',
          sharing_available: berth.sharing_available,
          nodi_value_per_day: '',
        });
      })
      .catch(() => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      setLoading(true);
      await ownerApi.updateBerth(berthId, {
        name: form.name,
      ...
      addToast({ type: 'success', message: 'Posto barca aggiornato con successo!' });
      router.push(`/owner/berths/${berthId}`);
    } catch {
      addToast({ type: 'error', message: 'Errore nell\'aggiornamento del posto barca' });
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) {
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div className="h-[600px] animate-pulse rounded-xl bg-sky-50" />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-sky-900">Modifica posto barca</h1>
          <p className="text-sm text-slate-500">Aggiorna le informazioni del posto barca</p>
        </div>
      </div>
